...
namespace Nes
{
	namespace Core
	{
		namespace Checksum
		{
			namespace Crc32
			{
				static dword ExpandCrc(const dword crc,const u8 data)
				{
				...
				static dword Iterate(const void* const mem,const ulong size,dword crc)
				{
					if (mem && size)
					{
						const u8* NST_RESTRICT data = static_cast<const u8*>(mem);
						const u8* const end = data + size;

						do
						{
							crc = ExpandCrc( crc, *data );
						}
						while (++data != end);
					}
				...
				static dword Iterate(Stream::In& in,ulong length,dword crc)
				{
					...
					{
						Stream::In stream( in );
						const ulong pos = stream.GetPos();

						do
						{
							crc = ExpandCrc( crc, stream.Read8() );
						}
						while (--length);
